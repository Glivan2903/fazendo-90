...
interface GridViewProps {
  ...
  getClassDataForDayAndHour: (day: number, hour: string) => Class[];
  openEditDialog: (classData: Class) => void;
  openNewDialog: (dayOffset: number, hour: string) => void;
}
    ...
    <>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-2">
        <div className="flex items-center space-x-2 w-full sm:w-auto mb-2 sm:mb-0">
          ...
          <Button variant="outline" size="icon" onClick={resetToCurrentWeek}>
                        ...
                        {classesForCell.length > 0 ? (
                          classesForCell.map((cls, idx) => (
                            <div key={idx} className="mb-1">
                              <ClassCard classData={cls} onClick={openEditDialog} />
                            </div>
                          ))
                        ) : (
                          <div 
                            ...
                            onClick={() => openNewDialog(day, hour)}
                          >
